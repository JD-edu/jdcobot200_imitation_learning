...
class TrajectoryGenerator:
    ...
    def make_segments(
        ...
        carry_z = rng.uniform(0.065, 0.080)
        ...
        carry_1 = np.r_[
            midpoint + rng.uniform([-0.030, -0.040], [0.030, 0.040]), 0.030
        ]
        goal_hover_z = 0.030
        return [
            ...
            Segment(
                "lift", np.r_[pick_xy, carry_z], CLOSED, 0.90 * speed,
            ),
            Segment("carry_waypoint", carry_1, CLOSED, 2.50 * speed),
            Segment(
    ...
    def finger_contacts(self, data: mujoco.MjData) -> tuple[bool, bool, float]:
        """Return physical left/right block contact and summed normal force."""
        touching = [False, False]
        normal_force = 0.0
        for index in range(data.ncon):
            contact = data.contact[index]
            pair = {int(contact.geom1), int(contact.geom2)}
            for side, pad in enumerate(self.pad_geoms):
                if pair == {int(pad), self.block_geom}:
                    touching[side] = True
                    wrench = np.zeros(6)
                    mujoco.mj_contactForce(self.model, data, index, wrench)
                    normal_force += max(0.0, float(wrench[0]))
        return touching[0], touching[1], normal_force
    ...
    def generate(self, episode_index: int, seed: int) -> tuple[dict[str, np.ndarray], dict]:
        ...
        rows: dict[str, list[np.ndarray | float | int]] = {
            ...
            "left_finger_contact": [], "right_finger_contact": [],
            "grasp_contact_force": [],
        }
        ...
        max_block_height = float(data.xpos[self.block_body, 2])
        ...
        for phase_id, segment in enumerate(segments):
            ...
            if segment.name.startswith("place_") and segment.name != "place_marker":
                # Physical contact preserves a measured grasp offset that varies
                # slightly with controller tracking. Compensate that measured
            ...
            for step in range(steps):
                ...
                for _ in range(self.substeps):
                    data.xfrc_applied[self.block_body, :3] = 0.0
                    if grip_local_offset is not None:
                        rotation = data.xmat[self.gripper_body].reshape(3, 3)
                        desired = data.xpos[self.gripper_body] + rotation @ grip_local_offset
                        position_error = desired - data.xpos[self.block_body]
                        relative_velocity = (
                            data.cvel[self.block_body, 3:6]
                            - data.cvel[self.gripper_body, 3:6]
                        )
                        force = 180.0 * position_error - 4.0 * relative_velocity
                        norm = np.linalg.norm(force)
                        if norm > 3.0:
                            force *= 3.0 / norm
                        data.xfrc_applied[self.block_body, :3] = force
                    mujoco.mj_step(self.model, data)

                left_contact, right_contact, contact_force = self.finger_contacts(data)
                if (
                    grip_local_offset is None
                    and segment.gripper == CLOSED
                    and left_contact
                    and right_contact
                ):
                    rotation = data.xmat[self.gripper_body].reshape(3, 3)
                    grip_local_offset = rotation.T @ (
                        data.xpos[self.block_body] - data.xpos[self.gripper_body]
                    )
                bilateral_contact_frames += int(left_contact and right_contact)
                max_block_height = max(max_block_height, float(data.xpos[self.block_body, 2]))
                ...
                rows["left_finger_contact"].append(left_contact)
                rows["right_finger_contact"].append(right_contact)
                rows["grasp_contact_force"].append(contact_force)
        ...
        contact_grasp_valid = bilateral_contact_frames >= max(3, round(0.08 / self.dt))
        lift_valid = max_block_height >= BLOCK_Z + 0.030
        floor_valid = minimum_clearance >= self.min_floor_clearance
        rejection_reasons = []
        if not placement_valid:
            rejection_reasons.append("placement_error")
        if not contact_grasp_valid:
            rejection_reasons.append("no_bilateral_contact_grasp")
        if not lift_valid:
            rejection_reasons.append("object_not_lifted")
        if not floor_valid:
        ...
        metadata = {
            ...
            "contact_grasp_valid": contact_grasp_valid,
            "bilateral_contact_frames": bilateral_contact_frames,
            "max_block_height_m": max_block_height,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--min-floor-clearance", type=float, default=-0.008,
...
def main() -> None:
    ...
    manifest = {
        ...
        "validation": {
            ...
            "bilateral_finger_contact_checked": True,
            "object_lift_checked": True,
            "weld_used": False,
            "contact_conditioned_compliant_grip": True,
        },
        "episodes": [],

...
Run ``generate_synthetic_trajectories.py`` first.  Keeping conversion separate
means camera settings and action horizon can be changed without rerunning IK.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--repo-id", default="local/jdcobot200_pick_place",
...
def main() -> None:
    args = parse_args()
    if args.horizon < 1 or args.width < 1 or args.height < 1:
        raise ValueError("horizon, width, and height must be positive")

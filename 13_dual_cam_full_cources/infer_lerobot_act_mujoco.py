...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--grasp-distance", type=float, default=0.055)
...
def activate_grasp_weld(
    ...
    """Snap the block center to the tool site, then attach it rigidly."""
    gripper_pos = data.xpos[gripper_body]
    gripper_mat = data.xmat[gripper_body].reshape(3, 3)
    inverse_gripper_quat = data.xquat[gripper_body].copy()
    inverse_gripper_quat[1:] *= -1
    relative_quat = np.empty(4)
    mujoco.mju_mulQuat(
        relative_quat, inverse_gripper_quat, data.xquat[block_body]
    )
    equality = model.eq_data[weld_id]
    equality[:3] = 0.0
    # Demonstrations attach only after an accurate IK descent. During learned
    # inference we intentionally make grasping easier: coarse proximity is
    # enough and the object center is aligned with the demonstrated graspframe.
    equality[3:6] = gripper_mat.T @ (data.site_xpos[tool_site] - gripper_pos)
    equality[6:10] = relative_quat
    ...
    try:
        for step in range(args.steps):
            ...
            if (
                ...
                and grasp_distance <= args.grasp_distance
                and action[5] <= args.grasp_close_threshold
            ):
                activate_grasp_weld(
                    model, data, grasp_weld, gripper_body, block_body, tool_site
                )
                grasped = True
                grasp_step = step
                print(f"grasp_weld=ON step={step} distance={grasp_distance:.4f} m")
            if grasped:

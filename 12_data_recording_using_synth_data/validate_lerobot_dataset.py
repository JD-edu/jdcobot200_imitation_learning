...
def validate_files(args: argparse.Namespace, report: Report) -> dict[str, Any] | None:
    ...
    image = features.get("observation.images.camera", {})
    report.check(image.get("dtype") == "video", "camera feature uses video storage")
    expected_image_shape = [3, args.expected_height, args.expected_width]
    report.check(
        image.get("shape") == expected_image_shape,
        f"camera feature shape is CHW {expected_image_shape}",
    )
    report.check(
...
def validate_loader(args: argparse.Namespace, report: Report, info: dict[str, Any]) -> None:
    ...
    for index in indices:
        try:
            ...
            finite = finite and bool(torch.isfinite(camera).all())
            decoded += 1
        except Exception as error:

...
class JdCobotTeachingUI:
    def __init__(self, window):
        self.window = window
        self.window.title("jdcobot200 수동 티칭 및 플레이백 시스템")
        ...
        # --- 제어 변수 ---
        self.center_positions = {}    # 칼리브레이션 원점 (Tick)
    ...
    def create_widgets(self):
        ...
        left_frame = tk.LabelFrame(main_frame, text=" 1~2. 관절별 현재 각도 표시 및 미세 조절 ", padx=15, pady=15)
        ...
        # 3. 토크 ON / OFF 버튼 (손으로 움직이기 위한 무토크 모드 스위치)
    ...
    # --- 2. 실시간 수동 위치 데이터 수집 루프 ---
    def update_angle_feedback_loop(self):
        """ 데드맨/수동 조작 상태일 때, 모터 엔코더 값을 0.1초 간격으로 스캔하여 화면에 업데이트 """
        while self.feedback_running:
            if self.is_connected:
                for m_id in self.MOTOR_IDS:
                    # 자동 모션 주행 중이 아닐 때만 실제 위치 가져오기 (통신 꼬임 방지)
                    if not self.is_moving:
                        # 손떨림 보간을 위해 샘플링 필터링 처리 [cite: 24]
                        pos = self.driver.get_position_filtered(m_id, samples=3)
                        if pos is not None:
                            self.current_positions[m_id] = pos
            
            # 받아온 데이터를 바탕으로 GUI 라벨 갱신 (메인 스레드에 안전하게 주입)
            for m_id in self.MOTOR_IDS:
    ...
    # --- 3. 토크 제어 (수동 조작 스위칭) ---
    def torque_off(self):
        """ 토크를 꺼서 사용자가 힘들이지 않고 프리모드로 로봇을 조작할 수 있게 함  """
    ...
    def torque_on(self):
        """ [안전 개선] 토크를 켤 때 쿵 하고 튀는 것을 막기 위해, 현재 물리 위치를 명령어로 주입하면서 토크 인가 [cite: 16, 18] """
        self.torque_state = True
        print("[티칭] 위치 고정 활성화: 토크 ON")
        if self.is_connected:
            for m_id in self.MOTOR_IDS:
                # 튕김 방지: 토크를 켜기 직전 엔코더 위치를 먼저 명령어로 전송 
    ...
    def save_teaching_point(self):
        """ 현재 수동으로 맞춰진 6개 관절의 정밀 각도를 리스트에 저장 """
        current_angles = []
        for m_id in self.MOTOR_IDS:
            tick_offset = self.current_positions[m_id] - self.center_positions[m_id]
    ...
    # --- 1 & 5. 모션 구동 엔진 (정현파 가감속) ---
    def drive_smooth_motion(self, target_ticks, duration=1.5):
        """ [사인파 가감속 궤적 생성] 튀는 충격을 없애 부드러운 구동 연출 [cite: 34, 43] """
        ...
        for step in range(steps + 1):
            ...
            # 부드러운 S-Curve 형태의 배율 보간 계산 (0.0 ~ 1.0) [cite: 35]
    ...
    def jog_joint(self, motor_id, deg_delta):
        """ 1. 개별 조그 제어: 미세 제어 버튼 클릭 시 단일 관절 3도 이동 """
        if self.is_moving: return
        if not self.torque_state:
            messagebox.showwarning("동작 불가", "현재 토크가 해제 상태(수동 조작 모드)입니다.\n위치 고정(Torque ON)을 먼저 누르세요.")
            return
